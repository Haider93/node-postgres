...
app.get('/insert_to_deleted/:sender/:receiver/:subject/:body/:date/:time',function(req,res){
	db.one('Insert into Deleted(sender,receiver,subject,body,date,time) values (${sender}, ${receiver}, ${subject}, ${body}, ${date}, ${time});',{
    sender: req.params.sender,
    receiver: req.params.receiver,
    subject: req.params.subject,
    body: req.params.body,
...
app.get('/delete/:email',function(req,res){
    db.one('delete from Email where receiver = ${email} || sender = ${email};',{
    email:req.params.email}).then(function(result){
    console.log(result);
    res.send(result);}).catch(function(error){
    res.send(error);
